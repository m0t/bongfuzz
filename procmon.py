...
class debugger_thread (threading.Thread):
	...
	def dbg_callback_user (self, dbg):
		...
		if not self.active:
			...
			self.process_monitor.log("debugger thread-%s detaching" % self.getName() )
			dbg.detach()
	...
	def run (self):
		...
		self.process_monitor.log("debugger thread-%s attaching to process %d" % ( self.getName(), self.pid) )
...
class process_monitor_pedrpc_server (pedrpc.server):
    ...
    #XXX: broken: remove or modify to single dbg thread
	def post_send (self):
		'''
		...
		av = self.dbg_thread.access_violation
		...
		if av:
			DEBUG("access violation found")
			while self.dbg_thread.isAlive():
				time.sleep(2)

		DEBUG("killin process")
		retcode = self.dbg_thread.inst.poll() 
		if retcode == None:
			self.dbg_thread.inst.kill()
		else:
			DEBUG("process already terminated, returncode %d, sending exception notify anyway" % retcode)
			self.dbg_thread.active = False
			return False
	...
	#mostly a dummy method, the way it is...
	def pre_send (self, test_number):
		...
		if not self.dbg_thread or not self.dbg_thread.isAlive():
			self.log("creating debugger thread", 5)
			self.dbg_thread = debugger_thread(self, self.proc_name, self.ignore_pid)
			self.dbg_thread.start()
			self.log("giving debugger thread 2 seconds to settle in", 5)
			time.sleep(2)


	def start_target (self, arg):
		'''
		...
		#need to pass exe (same for all) and args (many different)
		self.dbg_thread = debugger_thread(self, None)

		#start process
		self.dbg_thread.inst = subprocess.Popen("%s %s" % (self.start_commands, arg ))
		self.dbg_thread.pid = self.dbg_thread.inst.pid
		#XXX:debugger must attach before request but after a while...
		time.sleep(1)
		self.dbg_thread.start()
		#time for debugger to settle
		time.sleep(2)
